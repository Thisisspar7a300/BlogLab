    ...
    public class BlogRepository : IBlogRepository
    {
        ...
        public async Task<PagedResults<Blog>> GetAllAsnyc(BlogPaging blogPagging)
        {
            ...
            using (var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var multi = await connection.QueryMultipleAsync("Blog_GetAll",
                    new { 
                        offset = (blogPagging.page - 1) * blogPagging.PageSize,
                        PageSize = blogPagging.PageSize
                    }, commandType: CommandType.StoredProcedure))
                {
        ...
        public async Task<List<Blog>> GetAllFamousAsync()
        {
            ...
            using (var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                ...
                famouseBlogs = await connection.QueryAsync<Blog>("Blog_GetAllFamous",
                    new {  },
                    commandType: CommandType.StoredProcedure);
            }
            return famouseBlogs.ToList();
        }
